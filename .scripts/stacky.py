...
parser = argparse.ArgumentParser(description='A little script that makes managing my docker containers a bit easier')
parser.add_argument('-a', '--all', action='store_true', help='for deploying all stacks in all categories')
parser.add_argument('-c', '--category', nargs=1, help='for deploying all stacks in a category')
parser.add_argument('-s', '--stack', nargs=1, help='for deploying a stack in a category')
        ...
        if re.match(r'^[^.].+', directory):
            ...
            for file in files:
                if re.match(r'^[^.].*\.yml$', file):
                    fp = os.path.abspath(os.path.join(os.getcwd(), os.pardir, directory, file))
                    services = []
                    with open(fp, 'r') as f:
                        for line in f:
...
def stack_deploy(sn, cn):
    ...
    command_deploy = ' '.join(['sudo docker compose --file', path_yaml, '--env-file', path_env, 'up --detach --force-recreate'])
...
def print_list_items(l):
    count = len(l)
    for item in l:
        if count > 1 and count != len(l) and len(l) != 1: print(' ', end='')
        print(item, end='')
        if count > 1 and len(l) > 2: print(',', end='')
        if count == 2: print(' and ', end='') 
        count -= 1
...
def print_list_msg(l, p, t, g, a):
    if t == 'all' and g != 'category':
        ...
        if a == 'exists':
            print('there ', end='')
            if g == 'service':
                if len(list(l)) > 1: print('are', end='')
                else: print('is', end='')
            else:
                if len(list(l.keys())) > 1: print('are', end='')
                else: print('is', end='')
        elif a == 'start': print('starting', end='')
        elif a == 'stop': print('stopping', end='')
        print(' ', end='')
        if g == 'service': print(str(len(list(l))), end='')
        else: print(str(len(list(l.keys()))), end='')
        print(' ', end='')
        if a == 'exists': print('available ', end='')
        if g == 'category':
            print(g[:-1], end='')
            if len(list(l.keys())) > 1: print('ies', end='')
        elif g == 'stack' or g == 'service':
            print(g, end='')
            if g == 'stack':
                if len(list(l.keys())) > 1: print('s', end='')
            elif g == 'service':
                if len(list(l)) > 1: print('s', end='')
        print(' (', end='')
        print_list_items(list(l))
        print(') ', end='')
        if t != 'all': print('from', t, p, end='')
        print()
...
def main():
    ...
    if args.category:
        if args.category[0] in list(compost.keys()):
            print_list_msg(compost[args.category[0]], args.category[0], 'category', 'service', 'stop')
            list_stop(compost[args.category[0]], 'category')
            print_list_msg(compost[args.category[0]], args.category[0], 'category', 'service', 'start')
    ...
    if args.stack:
        found = False
        for category in list(compost.keys()):
            if args.stack[0] in list(compost[category].keys()):
                print_list_msg(compost[category][args.stack[0]], args.stack[0], 'stack', 'service', 'stop')
                stack_stop(compost[category][args.stack[0]])
                print_list_msg(compost[category][args.stack[0]], args.stack[0], 'stack', 'service', 'start')
                stack_deploy(args.stack[0], category)
                found = True
        if found == False: 
            print('invalid stack')
